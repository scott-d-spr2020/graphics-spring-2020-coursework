...
/*
	...
	a3_Demo_Materials_idle-input.c
...
void a3materialsCB_input_keyCharPress(a3_DemoState const* demoState, a3_Demo_Materials* demoMode, a3i32 asciiKey)
{
	switch (asciiKey)
	{
		// toggle render program
		a3demoCtrlCasesLoop(demoMode->render, materials_render_max, 'k', 'j');

		// toggle display program
		a3demoCtrlCasesLoop(demoMode->display, materials_display_max, 'K', 'J');

		// toggle active camera
		a3demoCtrlCasesLoop(demoMode->activeCamera, materials_camera_max, 'v', 'c');

		// toggle interpolation mode
		a3demoCtrlCasesLoop(demoMode->interp, materials_interp_max, 'V', 'C');

		// toggle pipeline mode
		a3demoCtrlCasesLoop(demoMode->pipeline, materials_pipeline_max, ']', '[');

		// toggle target
		a3demoCtrlCasesLoop(demoMode->targetIndex[demoMode->pass], demoMode->targetCount[demoMode->pass], '}', '{');

		// toggle pass to display
	//	a3demoCtrlCasesLoop(demoMode->pass, Materials_pass_max, ')', '(');
	case ')':
		a3demoCtrlIncLoop(demoMode->pass, materials_pass_max);
	case 'I':	// if toggle is switched, do the following
		if (demoState->skipIntermediatePasses)
		{
			a3demoCtrlIncClamp(demoMode->pass, materials_passBlend, materials_passComposite);
			a3demoCtrlIncClamp(demoMode->pass, materials_passComposite, materials_passScene);
			a3demoCtrlIncClamp(demoMode->pass, materials_passScene, -1);
		}
		break;
	case '(':
		a3demoCtrlDecLoop(demoMode->pass, materials_pass_max);
		if (demoState->skipIntermediatePasses)
		{
			a3demoCtrlDecClamp(demoMode->pass, materials_passBlend, materials_passComposite);
			a3demoCtrlDecClamp(demoMode->pass, materials_passComposite, materials_passScene);
			a3demoCtrlDecClamp(demoMode->pass, materials_passScene, -1);
			demoMode->pass = (demoMode->pass + materials_pass_max) % materials_pass_max;
		}
		break;
	}
