...
/*
	...
	a3_Demo_Materials_idle-render.c
...
void a3materials_render_controls(a3_DemoState const* demoState, a3_Demo_Materials const* demoMode,
	a3f32 const textAlign, a3f32 const textDepth, a3f32 const textOffsetDelta, a3f32 textOffset)
{
	...
	a3byte const* renderProgramName[materials_render_max] = {
		"Phong shading",
	};

	// forward display names
	a3byte const* displayProgramName[materials_display_max] = {
		"Texture",
		"Texture with outlines",
	};

	// active camera name
	a3byte const* cameraText[materials_camera_max] = {
		"Scene camera",
		"Shadow map light",
	};

	// pass names
	a3byte const* passName[materials_pass_max] = {
	...
	a3byte const* targetText_shadow[materials_target_shadow_max] = {
		"Depth buffer",
	};
	...
	a3byte const* targetText_composite[materials_target_composite_max] = {
		"Color target 0: FINAL DISPLAY COLOR",
	};
	a3byte const* targetText_bright[materials_target_bright_max] = {
		"Color target 0: FINAL DISPLAY COLOR",
		"Color target 1: Luminance",
	};
	a3byte const* targetText_blur[materials_target_blur_max] = {
		"Color target 0: FINAL DISPLAY COLOR",
	};
	a3byte const* const* targetText[materials_pass_max] = {
		targetText_shadow,
		targetText_scene,
	...
	a3byte const* interpText[materials_interp_max] = {
		"No interpolation",
	...
	// pipeline and target
	a3_Demo_Materials_RenderProgramName const render = demoMode->render;
	a3_Demo_Materials_DisplayProgramName const display = demoMode->display;
	a3_Demo_Materials_ActiveCameraName const activeCamera = demoMode->activeCamera;
	a3_Demo_Materials_PipelineName const pipeline = demoMode->pipeline;
	a3_Demo_Materials_PassName const pass = demoMode->pass;
	a3_Demo_Materials_TargetName const targetIndex = demoMode->targetIndex[pass];
	a3_Demo_Materials_TargetName const targetCount = demoMode->targetCount[pass];
	a3_Demo_Materials_InterpolationModeName const interp = demoMode->interp;

	// demo modes
	a3textDraw(demoState->text, textAlign, textOffset += textOffsetDelta, textDepth, col.r, col.g, col.b, col.a,
		"    Pipeline (%u / %u) ('[' | ']'): %s", pipeline + 1, materials_pipeline_max, pipelineText[pipeline]);
	a3textDraw(demoState->text, textAlign, textOffset += textOffsetDelta, textDepth, col.r, col.g, col.b, col.a,
		"    Display pass (%u / %u) ('(' | ')'): %s", pass + 1, materials_pass_max, passName[pass]);
	a3textDraw(demoState->text, textAlign, textOffset += textOffsetDelta, textDepth, col.r, col.g, col.b, col.a,
		"        Target (%u / %u) ('{' | '}'): %s", targetIndex + 1, targetCount, targetText[pass][targetIndex]);

	// lighting modes
	a3textDraw(demoState->text, textAlign, textOffset += textOffsetDelta, textDepth, col.r, col.g, col.b, col.a,
		"    Rendering mode (%u / %u) ('j' | 'k'): %s", render + 1, materials_render_max, renderProgramName[render]);
	a3textDraw(demoState->text, textAlign, textOffset += textOffsetDelta, textDepth, col.r, col.g, col.b, col.a,
		"    Display mode (%u / %u) ('J' | 'K'): %s", display + 1, materials_display_max, displayProgramName[display]);
	a3textDraw(demoState->text, textAlign, textOffset += textOffsetDelta, textDepth, col.r, col.g, col.b, col.a,
		"    Active camera (%u / %u) ('c' prev | next 'v'): %s", activeCamera + 1, materials_camera_max, cameraText[activeCamera]);

	// additional modes
	a3textDraw(demoState->text, textAlign, textOffset += textOffsetDelta, textDepth, col.r, col.g, col.b, col.a,
		"    Interpolation mode (%u / %u) ('C' | 'V'): %s", interp + 1, materials_interp_max, interpText[interp]);
	a3textDraw(demoState->text, textAlign, textOffset += textOffsetDelta, textDepth, col.r, col.g, col.b, col.a,
...
void a3materials_render(a3_DemoState const* demoState, a3_Demo_Materials const* demoMode)
{
	// pointers
	...
	const a3_DemoStateShaderProgram* displayProgram[materials_display_max] = {
		demoState->prog_drawTexture,
		demoState->prog_drawTexture_outline,
	};

	// framebuffers to which to write based on pipeline mode
	const a3_Framebuffer* writeFBO[materials_pass_max] = {
	...
	const a3_Framebuffer* readFBO[materials_pass_max][4] = {
		{ 0, },
	...
	a3_Demo_Materials_PipelineName const pipeline = demoMode->pipeline;
	a3_Demo_Materials_PassName const pass = demoMode->pass;
	a3_Demo_Materials_TargetName const targetIndex = demoMode->targetIndex[pass], targetCount = demoMode->targetCount[pass];
	a3_Demo_Materials_PassName currentPass;
	...
	// select shadow FBO
	currentPass = materials_passShadow;
	...
	currentPass = materials_passScene;
	currentWriteFBO = writeFBO[currentPass];
	switch (pipeline)
	{
		// shading with MRT
	...
	currentPass = materials_passComposite;
	currentWriteFBO = writeFBO[currentPass];
	a3framebufferActivate(currentWriteFBO);
	...
	currentPass = materials_passBright_2;
	currentWriteFBO = writeFBO[currentPass];
	currentReadFBO = readFBO[currentPass][0];
	a3framebufferActivate(currentWriteFBO);
	a3framebufferBindColorTexture(currentReadFBO, a3tex_unit00, 0);
	...
	currentPass = materials_passBlurH_2;
	currentWriteFBO = writeFBO[currentPass];
	currentReadFBO = readFBO[currentPass][0];
	a3framebufferActivate(currentWriteFBO);
	a3framebufferBindColorTexture(currentReadFBO, a3tex_unit00, 0);
	...
	currentPass = materials_passBlurV_2;
	currentWriteFBO = writeFBO[currentPass];
	currentReadFBO = readFBO[currentPass][0];
	a3framebufferActivate(currentWriteFBO);
	a3framebufferBindColorTexture(currentReadFBO, a3tex_unit00, 0);
	...
	currentPass = materials_passBright_4;
	currentWriteFBO = writeFBO[currentPass];
	currentReadFBO = readFBO[currentPass][0];
	a3framebufferActivate(currentWriteFBO);
	a3framebufferBindColorTexture(currentReadFBO, a3tex_unit00, 0);
	...
	currentPass = materials_passBlurH_4;
	currentWriteFBO = writeFBO[currentPass];
	currentReadFBO = readFBO[currentPass][0];
	a3framebufferActivate(currentWriteFBO);
	a3framebufferBindColorTexture(currentReadFBO, a3tex_unit00, 0);
	...
	currentPass = materials_passBlurV_4;
	currentWriteFBO = writeFBO[currentPass];
	currentReadFBO = readFBO[currentPass][0];
	a3framebufferActivate(currentWriteFBO);
	a3framebufferBindColorTexture(currentReadFBO, a3tex_unit00, 0);
	...
	currentPass = materials_passBright_8;
	currentWriteFBO = writeFBO[currentPass];
	currentReadFBO = readFBO[currentPass][0];
	a3framebufferActivate(currentWriteFBO);
	a3framebufferBindColorTexture(currentReadFBO, a3tex_unit00, 0);
	...
	currentPass = materials_passBlurH_8;
	currentWriteFBO = writeFBO[currentPass];
	currentReadFBO = readFBO[currentPass][0];
	a3framebufferActivate(currentWriteFBO);
	a3framebufferBindColorTexture(currentReadFBO, a3tex_unit00, 0);
	...
	currentPass = materials_passBlurV_8;
	currentWriteFBO = writeFBO[currentPass];
	currentReadFBO = readFBO[currentPass][0];
	a3framebufferActivate(currentWriteFBO);
	a3framebufferBindColorTexture(currentReadFBO, a3tex_unit00, 0);
	...
	currentPass = materials_passBlend;
	currentWriteFBO = writeFBO[currentPass];
	a3framebufferActivate(currentWriteFBO);
	for (i = 0, j = 4; i < j; ++i)
	...
	case materials_passScene:
		if (currentDisplayFBO->color && (!currentDisplayFBO->depthStencil || targetIndex < targetCount - 1))
			...
			a3framebufferBindDepthTexture(currentDisplayFBO, a3tex_unit00);
		break;
	case materials_passComposite:
	case materials_passBright_2:
	case materials_passBlurH_2:
	case materials_passBlurV_2:
	case materials_passBright_4:
	case materials_passBlurH_4:
	case materials_passBlurV_4:
	case materials_passBright_8:
	case materials_passBlurH_8:
	case materials_passBlurV_8:
	case materials_passBlend:
		a3framebufferBindColorTexture(currentDisplayFBO, a3tex_unit00, targetIndex);
		break;
	}
	...
	if (currentDisplayFBO)
	{
		...
		case materials_displayOutline:
			...
			a3framebufferBindColorTexture(currentReadFBO, a3tex_unit02, materials_scene_normal);
			break;
		}
	...
	if (demoState->displayWorldAxes && demoMode->pass != materials_passShadow)
	{
	...
	// individual objects
	if (demoState->displayObjectAxes && demoMode->pass != materials_passShadow)
	{
